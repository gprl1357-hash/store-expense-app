...
- 미인증 상태에서는 매장 버튼 완전 비노출, 인증된 매장만 드롭다운에 표시
- 최초 접속 시 로그인 방식 변경 공지 모달 ("72시간 동안 열지 않음" 선택적 숨김)
- 로그인/비밀번호 변경 5회 실패 시 15분 잠금 (무차별 대입 방지)
- 개선요청(피드백) 채널 1단계 MVP — 텍스트+사진 제보, Slack 즉시 알림
  (동영상·관리자 대시보드는 2단계 이후)
- 설정 탭에 로그아웃·매장 ID 표시

### Changed
- dev/운영 Supabase 프로젝트를 `VERCEL_ENV` 기준으로 완전 분리
- 로그인 화면 디자인 개선 (비밀번호 표시/숨김 토글 등)
- `/code-review`, `/simplify` 기반 코드 정리 (중복 헬퍼 통합, Supabase admin
  클라이언트 캐싱, 세션 조회 병렬화 등)

### Fixed
- 공지 모달이 로그인 전 초기 비밀번호를 노출하던 심각한 보안 결함 수정
- 로그인 실패 시 서버가 반환한 실제 에러 메시지(잠금 안내 포함)를
  화면에 표시하도록 수정 (기존엔 항상 일반 문구만 노출)

### Database
- `008_store_auth.sql` — 매장 인증 컬럼/테이블 추가, 기존 개방형 RLS
  정책 삭제 (service-role 전용으로 전환)
- `009_feedback.sql` — 피드백 테이블/스토리지 버킷 추가
- `010_login_lockout.sql` — 로그인 잠금 컬럼 추가
